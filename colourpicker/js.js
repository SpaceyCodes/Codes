...
const bottomtext = document.querySelector(".choicesb > p");
...
function workerstarter() {
    w.addEventListener("message", function() {
        if (x == 0) {
            ...
            result.innerHTML = `${score}/10 score | 20000ms`;
        }
        if (score == 10) {
            w.terminate();
            x = x - 3100;
...
function playerchoice(x) {
    if (robotchoice == x) {
        ...
        result.innerHTML = `${score}/10 score`;
        robot();
    } else {
        console.log("wrong");
        result.style.border= "4px solid red";
        result.innerHTML = `${score}/10 score`;
        robot();
    }
}
...
function starter() {
    startingbutton.addEventListener("click", function(){
        ...
        bottomtext.innerHTML = "Click to Stop";
    })
}
